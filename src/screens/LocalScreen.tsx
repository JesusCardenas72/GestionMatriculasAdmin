          ...
          await Promise.allSettled(
            actualizadas.map(async (solicitud) => {
              ...
              await actualizarRef.current(localExistente.localId, {
                ...
                repetidor: solicitud.repetidor ?? localExistente.repetidor ?? false,
                anulacion: solicitud.anulacion ?? localExistente.anulacion ?? false,
                ampliacion: solicitud.ampliacion ?? localExistente.ampliacion ?? false,
                ampliada: solicitud.ampliada ?? localExistente.ampliada ?? false,
  ...
  async function handleActualizarDesdeNube(local: MatriculaLocal, solicitudNube: Solicitud) {
    ...
    try {
      ...
      await actualizar(local.localId, {
        ...
        repetidor: solicitudNube.repetidor ?? local.repetidor ?? false,
        anulacion: solicitudNube.anulacion ?? local.anulacion ?? false,
        ampliacion: solicitudNube.ampliacion ?? local.ampliacion ?? false,
        ampliada: solicitudNube.ampliada ?? local.ampliada ?? false,
        asignaturas: asignaturasActualizadas,
  ...
  async function doSubirNube(m: MatriculaLocal) {
    if (m.rowId) {
        ...
        await actualizar(m.localId, {
          ...
          _pendienteSubida: false,
          _fueEditado: true,
          _nubeModificadoEn: new Date().toISOString(),
        });
      } else {

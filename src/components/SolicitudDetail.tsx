...
export default function SolicitudDetail({ config, solicitud, onDone }: Props) {
  ...
  const pdfQuery = usePdf(config, solicitud.tienePDF ? solicitud.rowId : null);
      ...
      <div className="bg-white rounded-xl shadow p-6">
        ...
        <section className="mt-6">
          ...
          <dl className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
            ...
            <Field label="Telefono" value={solicitud.telefono} />
            <Field
              label="Fecha nacimiento"
              value={formatDate(solicitud.fechaNacimiento)}
            />
            <Field label="Domicilio" value={solicitud.domicilio} />
            <Field label="Localidad" value={solicitud.localidad} />
            <Field label="Provincia" value={solicitud.provincia} />
            <Field label="CP" value={solicitud.cp} />
          </dl>
        </section>
        ...
        <section className="mt-6">
          ...
          <dl className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
            <Field
              ...
              value={formatDate(solicitud.fechaInscripcion)}
            />
            <Field
              label="Ensenanza y curso"
              value={solicitud.ensenanzaCurso}
            />
            <Field label="Especialidad" value={solicitud.especialidad} />
            <Field label="Forma de pago" value={solicitud.formaPago} />
            <Field label="Reduccion tasas" value={solicitud.reduccionTasas} />
            <Field
              label="Autorizacion imagen"
              value={solicitud.autorizacionImagen ? "Si" : "No"}
            />
            <Field
              label="Disponibilidad manana"
              value={solicitud.disponibilidadManana ? "Si" : "No"}
            />
            <Field label="Hora salida" value={solicitud.horaSalida} />
          </dl>
        </section>
        ...
        <section className="mt-6">
          ...
          {!solicitud.tienePDF && (
            <p className="text-sm text-slate-400">No hay PDF adjunto.</p>
          )}
          {solicitud.tienePDF && pdfQuery.isLoading && (
            <div className="flex items-center gap-2 text-sm text-slate-500">
              <Loader2 className="w-4 h-4 animate-spin" /> Descargando PDF...
            </div>
          )}
          {solicitud.tienePDF && pdfQuery.error && (
            <div className="text-sm text-red-600 flex items-start gap-2">
              <AlertCircle className="w-4 h-4 mt-0.5" />
              {(pdfQuery.error as Error).message}
            </div>
          )}
          {solicitud.tienePDF && pdfQuery.data && (
            <PdfViewer

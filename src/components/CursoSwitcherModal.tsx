...
import { clasificarCurso, cursoActualHoy, cursoProximoHoy } from "../utils/cursoContext";
...
export default function CursoSwitcherModal({ open, onClose }: Props) {
  ...
  // Asegurar que el curso actual y el próximo aparezcan aunque aún no tengan registros
  const cursosConBase = [...cursos];
  if (!cursosConBase.find((c) => c.curso === actual)) {
    cursosConBase.unshift({ curso: actual, totalRegistros: 0, archivadoEn: null, ultimaModificacion: "" });
  }
  if (proximo && !cursosConBase.find((c) => c.curso === proximo)) {
    cursosConBase.unshift({ curso: proximo, totalRegistros: 0, archivadoEn: null, ultimaModificacion: "" });
  }
